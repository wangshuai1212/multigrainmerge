...
#include "GrainACInterface.h"

registerMooseObject("multigrainmergeApp", GrainACInterface);

InputParameters
GrainACInterface::validParams()
{
...
GrainACInterface::GrainACInterface(const InputParameters & parameters)
  : DerivativeMaterialInterface<JvarMapKernelInterface<Kernel>>(parameters),
    ...
    _dkappadarg(_n_args),
    _gradarg(_n_args),
    _mo1(getMaterialProperty<Real>("Mo"))
{
...
RealGradient
GrainACInterface::nablaLPsi()
{
  // sum is the product rule gradient \f$ \nabla (L\psi) \f$
  RealGradient sum = _L[_qp] * _mo1[_qp] * _grad_test[_i][_qp];
...
RealGradient
GrainACInterface::kappaNablaLPsi()
{
  return _kappa[_qp] * nablaLPsi();
}

Real
GrainACInterface::computeQpResidual()
{
  return _grad_u[_qp] * kappaNablaLPsi();
}

Real
GrainACInterface::computeQpJacobian()
{
  // dsum is the derivative \f$ \frac\partial{\partial \eta} \left( \nabla (L\psi) \right) \f$
  RealGradient dsum =
      (_dkappadop[_qp] * _L[_qp] * _mo1[_qp] + _kappa[_qp] * _dLdop[_qp]) * _phi[_j][_qp] * _grad_test[_i][_qp];
...
Real
GrainACInterface::computeQpOffDiagJacobian(unsigned int jvar)
{
  // get the coupled variable jvar is referring to
  const unsigned int cvar = mapJvarToCvar(jvar);

  // dsum is the derivative \f$ \frac\partial{\partial \eta} \left( \nabla (L\psi) \right) \f$
  RealGradient dsum = ((*_dkappadarg[cvar])[_qp] * _L[_qp] * _mo1[_qp] + _kappa[_qp] * (*_dLdarg[cvar])[_qp]) *
